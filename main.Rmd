...
author:
    - Ian Chen^[Writing, Analysis, EDA, Model Building]
    - Yan Yi Lance Du^[Reviewing and Editing, Analysis, EDA, Model Building]
...
The only predictors associated with decreased fatigue is getting enough sleep.
...
```{r fig1, echo = F, fig.cap="\\label{figs:fig1} Distribution of Response Variable on the Training Dataset. The x-axis describes the median response time in milliseconds. The y-axis describes the number of results."}
...
```{r fig2, echo = F, fig.cap="\\label{figs:fig2} Correlation Matrix"}
df <- data.train[, c("Time", "StressN", "FatigueN", "NoiseLevel", "AvgHoursExercise", "AvgSleepTime", "LastNightSleep")]
cor_heatmap <- cor(df)
...
```{r, echo = F, results = 'hide'}
fit5 <- lm(StressN ~ LastNightSleep + AvgHoursExercise + ActiveSports + ActiveGame, data = data.train)
summary(fit5)
vif(fit5)
```

```{r, echo = F, results = 'hide'}
fit6 <- lm(FatigueN ~ LastNightSleep + AvgHoursExercise + ActiveSports + ActiveGame, data = data.train)
summary(fit6)
vif(fit6)
```
...
Getting more sleep is also strongly related to decreases in fatigue (p = 0.0075).
...
```{r echo = F, results = 'hide'}
fit <- lm(StressN ~ Time * Mouse + Distraction + NoiseLevel + ActiveSports + ActiveGame + LastNightSleep + AvgHoursExercise, data = data.train)
summary(fit)
anova(fit)
```
...
Running backwards variable selection using AIC, we find that we should use the predictors ActiveSports and Distraction.
...
```{r echo = F, results = 'hide'}
fit <- lm(FatigueN ~ Time * Mouse + Distraction + NoiseLevel + ActiveSports + ActiveGame + LastNightSleep + AvgHoursExercise, data = data.train)
summary(fit)
anova(fit)
```
...
fitFatigue <- lm(FatigueN ~ LastNightSleep + Distraction, data = data.train)
summary(fitFatigue)
```
...
Running backwards variable selection using AIC, we find that we should use the predictors LastNightSleep and Distraction to predict Fatigue. 
Our final model, using predictors LastNightSleep and Distraction, is
$$
    Fatigue \sim
    \begin{cases}
        3.160 - 0.118 * LastNightSleep & \text{If no distractions} \\
        3.485 - 0.118 * LastNightSleep & \text{if distractions} \\
    \end{cases}
$$
...
fit3 <- lm(FatigueN ~ LastNightSleep, data = data.train)
